...
public abstract class AbstractSecurityWebSocketMessageBrokerConfigurer
        implements SmartInitializingSingleton, WebSocketMessageBrokerConfigurer {
    ...
    private SecurityExpressionHandler<Message<Object>> defaultExpressionHandler =
    ...
    public ChannelSecurityInterceptor inboundChannelSecurity() {
        ChannelSecurityInterceptor channelSecurityInterceptor =
                new ChannelSecurityInterceptor(inboundMessageSecurityMetadataSource());
    ...
    public void afterSingletonsInstantiated() {
        ...
        for (Object object : mappings.values()) {
            if (object instanceof SockJsHttpRequestHandler) {
                ...
                List<HandshakeInterceptor> interceptorsToSet =
                        new ArrayList<>(handshakeInterceptors.size() + 1);
                ...
                throw new IllegalStateException(
                        "Bean "
                                + beanName
                                + " is expected to contain mappings to either a SockJsHttpRequestHandler "
                                + "or a WebSocketHttpRequestHandler but got "
                                + object);
            }
        }
